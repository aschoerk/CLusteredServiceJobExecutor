...
class ApiTests {
    ...
    static Logger logger = LoggerFactory.getLogger("ApiTests");

    TestContainer container;

    @Bean
    BeansFactory createBeansFactory() {
        return new TestBeansFactory();
    }

    public TestContainer getContainer() {
        if(this.container == null) {
            this.container = new TestContainer("dummyNodes", Clock.systemUTC());
        }
    ...
    private static void checkTests(final int expectedDone, final int expectedRunning, final int expectedDelayed,
        ...
        Assertions.assertEquals(0, SpringTestStep.collisionsDetected.get(), "collisions in threadscoped step");
    }
    ...
    @Autowired
    SpringTestJob springTestJob;

    @Test
    void simpleGroupTest() throws InterruptedException {
        final TestBeansFactory testBeansFactory = new TestBeansFactory();
        Engine engine = Providers.get().createTestEngine(getContainer(), testBeansFactory);
        engine.register(springTestJob);
        Transport transport = engine.create(springTestJob, "group", new TestContext("group"));
    ...
    void cdiTest(boolean doremote, int loops, int expectedDone, int expectedRunning, int expectedDelayed,
        ...
        SpringTestStep.successesInSequence = successesInSequence;
        final TestBeansFactory testBeansFactory = new TestBeansFactory();
        Engine engine = Providers.get().createTestEngine(getContainer(), testBeansFactory);
        String[] groups = generateGroupNames(groupNo);
        engine.register(springTestJob);
        if(doremote) {
            engine.register(createRemoteTestExecutor(springTestJob));
        }
    ...
    void submitTest() throws InterruptedException {
        ...
        Future<?> future = getContainer().submitInWorkerThread(
                () -> ints[0]++
        );
        while (!future.isDone()) {
            Thread.sleep(100);
    ...
    void reviverTest(int jobnumber, int fixedEngineNumber, int engineNumber, int revivalAfter, int creationTime,
        ...
        getContainer().setWorkerThreadPoolSize((fixedEngineNumber + engineNumber) * 20);
        SpringTestStep.successesInSequence = successesInSequence;
        Set<String> revivedEngines = new HashSet<>();
        final TestBeansFactory testBeansFactory = new TestBeansFactory();
        Engine[] fixedEngines = new Engine[fixedEngineNumber];
        for (int i = 0; i < fixedEngineNumber; i++) {
            fixedEngines[i] = Providers.get().createTestEngine(getContainer(), testBeansFactory);
            fixedEngines[i].register(springTestJob);
        }
        List<Engine> engines = new ArrayList<>();
        for (int i = 0; i < engineNumber; i++) {
            engines.add(Providers.get().createTestEngine(getContainer(), testBeansFactory));
            engines.get(i).register(springTestJob);
        }
    ...
    private void createJob(final String[] groupIds, final int createdCount, final Engine creatingEngine) {
        if(groupIds.length > 0) {
            ...
            creatingEngine.create(springTestJob, groupId, new TestContext(groupId));
        }
        else {
            creatingEngine.create(springTestJob, new TestContext());
        }
    }
    ...
    private void handleRevival(final TestBeansFactory testBeansFactory,
        ...
        if(toRevive.getStartupTime().plus(Duration.ofSeconds(20)).isBefore(Instant.now())) {
            ...
            if(revivedEngines.contains(engineName)) {
                ...
                getContainer().submitInWorkerThread(
                        () -> {
                            toRevive.stop();
                            final EngineImpl newEngine = (EngineImpl) Providers.get().createTestEngine(getContainer(), testBeansFactory);
                            engines.set(currentEngine, newEngine);
                            engines.get(currentEngine).register(springTestJob);
    ...
    private Instant outputInfo(final TestBeansFactory testBeansFactory, final Set<String> revivedEngines, final Set<Map.Entry<Pair<String, String>, TransportImpl>> contexts, Instant outputTimestamp) {
        final Duration maxDelay = getContainer().getConfiguration().getMaxDelayOfStateMessages();
        if(logger.isInfoEnabled() && Instant.now().isAfter(outputTimestamp.plus(Duration.ofSeconds(3)))) {
            ...
            logger.info("rn: {} dlyd: {} dn: {} er: {} grp: {} rnSnt: {} dlydSnt: {} grpSnt: {} elpsdR: {} elpsedDd: {} rvivR: {} rvivD: {} calld: {} coll: {} ",
                    ...
                    elapsedRunning[0], elapsedDelayed[0], revivableRunning[0], revivableDelayed[0], SpringTestStep.stepEntered,
                    SpringTestStep.collisionsDetected
            );
            ...
            logger.info("t: {} e: {} l: {} q: {} rn: {} dlyd: {} dn: {} er: {} grp: {} rnSnt: {} dlydSnt: {} grpSnt: {} elpsdR: {} elpsedDd: {} rvivR: {} rvivD: {} calld: {} coll: {}",
                    SpringTestStep.staticThreadCount, SpringTestStep.stepEntered.get(), SpringTestStep.stepLeft.get(), this.getContainer().workerThreadQueue.size(),
                    stateMap.get(RUNNING), stateMap.get(DELAYED), stateMap.get(DONE), stateMap.get(ERROR), stateMap.get(GROUP),
                    stateCounts.get(State.RUNNING).get(), stateCounts.get(State.DELAYED).get(), stateCounts.get(State.GROUP).get(),
                    elapsedRunning[0], elapsedDelayed[0], revivableRunning[0], revivableDelayed[0], SpringTestStep.stepEntered,
                    SpringTestStep.collisionsDetected
