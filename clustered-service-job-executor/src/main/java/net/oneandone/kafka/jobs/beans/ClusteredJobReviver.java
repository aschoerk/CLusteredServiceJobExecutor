...
public class ClusteredJobReviver extends StoppableBase implements ClusterTask {
    ...
    private void receiveJobState(final Map<String, Object> consumerConfig) {
        logger.info("Starting to receive Jobstates");
    ...
    @Override
    public void call() {
        logger.trace("E: {} call", beans.getEngine().getName());
        if (stateInitCompleted) {
            ...
            statesToRevive.stream().forEach(s -> {
                ...
                if(jobData != null) {
                    logger.warn("E: {} Reviving: {}", beans.getEngine().getName(), jobData.jobData());
        ...
        logger.trace("E: {} call end", beans.getEngine().getName());
    }
